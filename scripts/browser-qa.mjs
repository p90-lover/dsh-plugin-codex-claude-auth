...
try {
  ...
  await native.getByRole('button', { name: '500K', exact: true }).click()
  await native.getByRole('button', { name: '套用上下文', exact: true }).click()
  await native.getByText('設定已由宿主確認。下一個請求會使用新的有效上限。').waitFor()
  const saved = await native.evaluate(async () => (await fetch('/plugins/dsh-oauth-model-providers/oauth/openai-codex-oauth/status')).json())
  assert.equal(saved.contextWindow.selected, 500000)
